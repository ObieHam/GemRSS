      ...
      <div className="bg-slate-900 border-2 border-slate-700 rounded-3xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="flex items-center justify-between p-4 bg-slate-800 rounded-xl cursor-pointer">
              <div>
                ...
                <div className="text-slate-400 text-sm">Automatically add words when clicked</div>
              </div>
              ...
              <input type="checkbox" className="hidden" checked={settings.autoSave} onChange={(e) => updateSettings({ ...settings, autoSave: e.target.checked })} />
            </label>
          </div>
          ...
          <div>
            <label className="flex items-center justify-between p-4 bg-slate-800 rounded-xl cursor-pointer">
              <div>
                <div className="text-white font-bold">Highlight new words</div>
                <div className="text-slate-400 text-sm">Glow potential vocabulary in the reader</div>
              </div>
              <div className={`w-14 h-8 rounded-full transition-colors ${settings.highlightNewWords ? 'bg-indigo-500' : 'bg-slate-700'} relative`}>
                <div className={`w-6 h-6 bg-white rounded-full absolute top-1 transition-transform ${settings.highlightNewWords ? 'translate-x-7' : 'translate-x-1'}`}></div>
              </div>
              <input type="checkbox" className="hidden" checked={settings.highlightNewWords} onChange={(e) => updateSettings({ ...settings, highlightNewWords: e.target.checked })} />
            </label>
          </div>
        </div>
      </div>
